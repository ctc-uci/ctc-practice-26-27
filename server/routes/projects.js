...
projectsRouter.post("/", async (req, res) => {
    try {
        ...
        const project = await db.one(
            `INSERT INTO ss_project_info (npo_id, start_year, end_year, project_leads)
             VALUES ($1, $2, $3, $4)
             RETURNING *;`,
...
projectsRouter.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const project = await db.oneOrNone(
            `DELETE FROM ss_project_info WHERE id = $1 RETURNING *;`,
            [id]
        );
        if (!project) return res.status(404).send(`No project with that id`);
        return res.status(200).json(keysToCamel(project));
